...
class ResidualBlock1(nn.Module):
    ...
    def forward(self, x, original):
        ...
        # 确保 original 通过 shortcut 后形状和 out 一致
        shortcut_output = self.shortcut(original)
        # 检查形状是否匹配
        if out.shape != shortcut_output.shape:
            raise ValueError(f"Shape mismatch: out shape {out.shape}, shortcut_output shape {shortcut_output.shape}")
        # 残差连接
        out += shortcut_output
        out = self.relu(out)
        return out
...
if __name__ == "__main__":
    ...
    # 创建堆叠的残差块模型
    model = StackedResidualBlocks1(in_channels=64, num_blocks=2)  # 堆叠 2 次
