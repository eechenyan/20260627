...
def train(train_loader, cfg, test_scene_id):
    ...
    os.makedirs('./log30/' + str(test_scene_id[0]) + '_' + str(test_scene_id[1]), exist_ok=True)
    sys.stdout = open('./log30/' + str(test_scene_id[0]) + '_' + str(test_scene_id[1])
    ...
    for idx_epoch in range(0, cfg.n_epochs):
        ...
        save_ckpt({'epoch': idx_epoch + 1,'state_dict': net.state_dict(),'loss': loss_list,},
                  save_path = './log30/' + str(test_scene_id[0]) + '_' + str(test_scene_id[1]) + '/',
                  filename='PVBLiF_epoch' + str(idx_epoch + 1) + '.pth.tar')
        load_model_path = './log30/'+ str(test_scene_id[0]) + '_' + str(test_scene_id[1]) + \
        ...
        if (idx_epoch + 1) != cfg.n_epochs:
            os.system('rm -r ./log30/' + str(test_scene_id[0]) + '_' + str(test_scene_id[1]) + '/'
                      + 'PVBLiF_epoch' + str(idx_epoch + 1) + '.pth.tar')
    ...
    output_file_path = './log30/' + str(test_scene_id[0]) + '_' + str(test_scene_id[1]) + '/weights.xlsx'
    weights_df.to_excel(output_file_path, index=False, header=False)  # 保存到 Excel 文件


def save_ckpt(state, save_path='./log30', filename='checkpoint.pth.tar'):
