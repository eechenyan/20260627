...
def parse_args():
    ...
    parser.add_argument('--trainset_dir', type=str, default="/project/pythonProject3/Data/Sci_PVBLiF_SHU_9_9_32_32_patch/")

    return parser.parse_args()

def train(train_loader, cfg, test_scene_id):

    os.makedirs('./logTF0618/' + str(test_scene_id[0]) + '_' + str(test_scene_id[1]), exist_ok=True)
    sys.stdout = open('./logTF0618/' + str(test_scene_id[0]) + '_' + str(test_scene_id[1])
    ...
    for idx_epoch in range(0, cfg.n_epochs):
        ...
        save_ckpt({'epoch': idx_epoch + 1,'state_dict': net.state_dict(),'loss': loss_list,},
                  save_path = './logTF0618/' + str(test_scene_id[0]) + '_' + str(test_scene_id[1]) + '/',
                  filename='PVBLiF_epoch' + str(idx_epoch + 1) + '.pth.tar')
        load_model_path = './logTF0618/'+ str(test_scene_id[0]) + '_' + str(test_scene_id[1]) + \
        ...
        scheduler.step()
def save_ckpt(state, save_path='./logTF0618', filename='checkpoint.pth.tar'):
