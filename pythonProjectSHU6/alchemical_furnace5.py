...
class Network(nn.Module):
    ...
    def __init__(self):
        ...
        self.SFE_conv1 = nn.Conv3d(in_channels=1, out_channels=64, kernel_size=(9,1,1), stride=(1,1,1), dilation=(9,1,1))#dilation=(7,1,1)重点解释 9意思是卷积核膨胀7倍，中间为0，即1和8才有数字，其他都是0。1代表不变
        ...
        self.TF_AFE = TransformerBlock(dim=64, depth=3)
        self.TF_SFE = TransformerBlock(dim=64, depth=3)
        self.TF_EFE_H = TransformerBlock(dim=64, depth=3)
        self.TF_EFE_V = TransformerBlock(dim=64, depth=3)
        #-------------------------------------------------------------------------------------------------------------------
        # part3-------------------------------------------------------------------------------------------------------------------------
        # 这部分是三层残差网络
        # 第一层（堆叠残差块）
    ...
    def forward(self,x):
        ...
        AFE_CBAM_out = self.TF_AFE(AFE_out)
        SFE_CBAM_out = self.TF_SFE(SFE_out)
        EFE_H_CBAM_out = self.TF_EFE_H(EFE_H_out)
        EFE_V_CBAM_out = self.TF_EFE_V(EFE_V_out)
