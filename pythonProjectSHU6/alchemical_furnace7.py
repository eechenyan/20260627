...
class Network(nn.Module):
    ...
    def __init__(self):
        ...
        # part4------------------------------------------------------------------------------------------------------------------
        # 这部分融合特征和进行金字塔拼接
        self.GCNet_AFE = GCNet(in_channels=64, output_channels=1)  # 输入特征数为192 #hidden_dim可以是64 num_layers可以是2
        self.GCNet_SFE = GCNet(in_channels=64, output_channels=1)  # 输入特征数为192# 应为 (1, hidden_dim, 32, 32)
        self.GCNet_EFE_H = GCNet(in_channels=64, output_channels=1)  # 输入特征数为192# 应为 (1, hidden_dim, 5, 32)
        self.GCNet_EHE_V = GCNet(in_channels=64, output_channels=1)  # 输入特征数为192  # 应为 (1, hidden_dim, 32, 5)
        # 金字塔池化拼接
    ...
    def forward(self, x):
        ...
        AFE_CBAM_out_Rcat = AFE_CBAM_out
        SFE_CBAM_out_Rcat = SFE_CBAM_out
        EFE_H_CBAM_out_Rcat = EFE_H_CBAM_out
        EFE_V_CBAM_out_Rcat = EFE_V_CBAM_out
